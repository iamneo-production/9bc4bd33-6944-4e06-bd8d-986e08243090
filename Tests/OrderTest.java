...
public class OrderTest{
   ...
   public void testCreateOrders(){
       OrderModel order=new OrderModel(78,"snigdha","02-03-2021","regrg",(long)568952456,67,9,89.78,"Completed");
       OrderModel order2=new OrderModel(78,"snigdha","02-03-2021","regrg",(long)568952456,67,9,89.78,"Completed");
...
public void testUpdateOrder() {
    OrderModel order = repo.findById(1).orElse(null);
    order.setName("Roja");
    repo.save(order);
    OrderModel updatedOrder = repo.findById(1).orElse(null);    
...
public void testDeleteOrder() {
    ...
    repo.deleteById(order.getId());
     OrderModel deletedOrder = repo.findById(1).orElse(null);
    assertThat(deletedOrder).isNull();           
}
@Test
public void testDeleteOrders() {
    OrderModel order = repo.findById(1).orElse(null);
    OrderModel order2 = repo.findById(2).orElse(null);
    repo.deleteById(order.getId());
    repo.deleteById(order2.getId());
     OrderModel deletedOrder; 
     deletedOrder = repo.findById(1).orElse(null);
     deletedOrder = repo.findById(2).orElse(null);
    assertThat(deletedOrder).isNull();            
}
}